...
### 💡 Trình diễn 4: Tự động dự phòng chịu lỗi (Auto Failover) và Đồng bộ ngược khôi phục (Sync-Back) trên Web UI
Đây là phần nâng cấp đặc biệt chứng minh khả năng chịu lỗi và tính tự phục hồi của Cluster:
*   **Thao tác thực hiện:**
    1.  Mở Dashboard Web UI (Tab "Cụm Phân Tán"). Tất cả 5 nút đều hiển thị màu xanh lá (ONLINE).
    2.  Mở PowerShell/CMD và tắt tiến trình **Shard 1 Master** (cổng `4001`) bằng cách tắt cửa sổ terminal đó (hoặc bấm `Ctrl+C` tại terminal chạy server đó).
    3.  Trên Dashboard UI, sau 5 giây nút **Shard 1 - Master** sẽ tự động chuyển sang màu đỏ (OFFLINE).
        *   Màn hình Terminal Monitor Log ở dưới chân trang lập tức ghi nhận lỗi:
            `🔴 CẢNH BÁO SỰ CỐ: Nút Shard 1 Master đã mất kết nối (Offline)!` và 
            `⚠️ Chế độ dự phòng (Failover) được kích hoạt: Router Proxy sẽ chuyển tiếp đọc/ghi tới Slave.`
    4.  Tại bảng điều khiển bên trái, thử Thêm Cặp Dữ Liệu có key thuộc Shard 1 (Ví dụ: Key `banana`, Value `Chuối chín vàng`). Click **--> Ghi Phân Phối**.
        *   **Kết quả:** Client ghi nhận thành công!
        *   Terminal Monitor Log in log màu vàng: `⚠️ PHÁT HIỆN SỰ CỐ: Master Shard offline. Router đã chuyển vùng (Failover) ghi sang Slave thành công!`.
        *   Trong phần *"Xem Phân Mảnh Vật Lý Chi Tiết"* ở cuối trang Dashboard, cột **Shard 1 - Master** hiển thị `OFFLINE`, nhưng cột **Shard 1 - Slave** hiển thị Key `banana` đã được lưu trữ vật lý thành công!
    5.  Khởi động lại **Shard 1 Master** (Mở CMD/PowerShell và chạy lệnh: `py -c "from pupdb.rest import APP; APP.run(port=4001)"` hoặc chạy lại `start_cluster.py` nếu đã tắt tất cả).
    6.  Khi Master 1 kết nối lại, Dashboard phát hiện và chuyển sang màu xanh lá (ONLINE).
        *   Terminal Monitor Log lập tức in log: `🟢 Kết nối phục hồi: Nút Shard 1 Master hoạt động trở lại (Online).` và `🔄 TỰ ĐỘNG ĐỒNG BỘ: Shard 1 Master kích hoạt Sync-Back dữ liệu từ Slave...`.
        *   Sau 1.5 giây, log báo: `✅ ĐỒNG BỘ THÀNH CÔNG: Shard 1 Master đã lấy đủ dữ liệu mới và phục hồi nhất quán dữ liệu!`.
        *   Cột dữ liệu thô của **Shard 1 - Master** đã tự động xuất hiện Key `banana` đồng bộ hoàn tất!
*   **Từ khóa ăn điểm:** *High Availability (Tính khả dụng cao)*, *Proxy Failover (Chuyển vùng dự phòng)*, *Failback / Sync-Back (Đồng bộ ngược phục hồi)*, *Eventual Consistency (Tính nhất quán cuối cùng)*.
